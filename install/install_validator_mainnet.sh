...
cat > /home/solana/create_accounts <<EOF
  ...
  solana-keygen pubkey /home/solana/.secrets/validator-keypair.json
  ...
  sleep 10
  solana create-stake-account  --withdraw-authority $(solana-keygen pubkey ~/.secrets/withdrawer-stake-keypair.json) ~/.secrets/validator-stake-keypair.json 100.1
  sleep 10
  solana balance ~/.secrets/validator-stake-keypair.json
  solana delegate-stake ~/.secrets/validator-stake-keypair.json ~/.secrets/vote-account-keypair.json
  solana-keygen pubkey ~/.secrets/validator-stake-keypair.json
EOF
